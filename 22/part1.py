...
with open("input.txt") as file:
    for i, line in enumerate(file):
        p0, p1 = (tuple(map(int, p.split(","))) for p in
                  line.strip().split("~"))
        cubes[i] = (p0, p1)
...
grid = dict(((x, y, 0), -1) for x in range(gminx, gmaxx+1) for y in
            range(gminy, gmaxy+1))

# lower cubes (sorted from lowest to highest) until they collide
